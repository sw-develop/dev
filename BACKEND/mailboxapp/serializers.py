...
class ListMailBoxSerializer(serializers.ModelSerializer):
    ...
    check_mailbox_today = serializers.SerializerMethodField()

    class Meta:
        model = MailBox
        fields = ['id', 'link_title', 'mailbox_link', 'open_date', 'number_of_letter', 'checked', 'check_mailbox_today']

    def get_number_of_letter(self, obj):
        return obj.number_of_letter()

    def get_check_mailbox_today(self, obj):  # obj : mailbox 객체
        return obj.user.check_mailbox_today()
